...
<br>
<br>
<h2 class="text-center my-5">SURAT PENAWARAN</h2>
...
<div class="container">
    <div class="row">
        <div class="col-md-5 d-block justify-content-center">
           ...
           <b>Keterangan : </b><br>
            <li>Bagi yang masih bingung untuk cara pengisiannya, bisa nonton video diatas </li>
...
memasuki Dunia Kerja atau yang ingin mendalami dari Nol sampai Tuntas. Pendaftaran <b class="text-danger"><i><u>setiap hari</u></i></b> </small><p></hr>
                            </div>
                            <div class="form-group">
...
<script>
    ...
    $('[name="pilihan-training-inkubator"]').change(function(){
        $("#kelas-terpilih").html($('[name="pilihan-training-inkubator"]').find(':selected').text());
        setupKontribusi();
    });
...
<script>
    function submitForm(){
        ...
        $.post('<?= site_url('Suratpenawaran/suratPenawaranProcess')?>', JSON.stringify(finalData), function(res){
            $('#loading-modal').modal('hide');
            alert(res.message);
            if(res.error == 200){
                location.assign("<?= site_url('/surat-penawaran')?>/" + res.nextid);
            }
        }, 'json');
    }
</script>
